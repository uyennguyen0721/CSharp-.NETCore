...
namespace Register_Login_Logout.Areas.Identity.Pages.Account
{
    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPost(string provider, string returnUrl = null)
        {
            ...
            var listprovider = (await _signInManager.GetExternalAuthenticationSchemesAsync()).ToList(); //Lấy các dịch vụ xác thực ngoài
        ...
        public async Task<IActionResult> OnGetCallbackAsync(string returnUrl = null, string remoteError = null)
        {
            ...
            var info = await _signInManager.GetExternalLoginInfoAsync(); //Lấy thông tin đăng nhập từ tài khoản của phiên hiện tại (sau khi được chuyển hướng từ dịch vụ ngoài về ứng dụng)
            if (info == null)
            {
            ...
            var result = await _signInManager.ExternalLoginSignInAsync(info.LoginProvider, info.ProviderKey, isPersistent: false, bypassTwoFactor: true); //Đăng nhập tài khoản bằng thông tin từ dịch vụ ngoài
            ...
            else
            {
                ...
                var userExisted = await _userManager.FindByLoginAsync(info.LoginProvider, info.ProviderKey); //Tìm User theo thông tin liên kết với dịch vụ ngoài
                if (userExisted != null)
                {
        ...
        public async Task<IActionResult> OnPostConfirmationAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if ((userWithexternalMail != null) && (Input.Email == externalMail))
                {
                    ...
                    var resultAdd = await _userManager.AddLoginAsync(userWithexternalMail, info); //Liên kết User với dịch vụ ngoài
